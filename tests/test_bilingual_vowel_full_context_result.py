...
def test_full_context_result_is_complete_zero_api_and_nonpromotional() -> None:
    result = _load()

    assert result["record_sha256"] == _semantic_hash(result)
    assert result["classification"] == (
        "full_context_candidate_screen_complete_no_product_promotion"
    )
    assert result["candidate_cell_count"] == 13
    assert result["logical_slot_count"] == result["measured_slot_count"] == 39
...
def test_full_context_result_records_one_rescue_without_hiding_failures() -> None:
    result = _load()
    cells = result["cell_summaries"]

    assert result["cell_classification_counts"] == {
        "exact_category_pass": 1,
        "directional_only_pass": 0,
        "fail": 12,
    }
    assert result["rescued_cell_count"] == 1
    assert result["automatic_human_qc_eligible_cell_count"] == 1
    rescued = [row for row in cells if row["candidate_classification"] != "fail"]
    assert [
        (row["voice_id"], row["rule_id"], row["candidate_classification"])
        for row in rescued
    ] == [("am_michael", "enpt.reduced_i_i", "exact_category_pass")]
    assert Counter(row["candidate_classification"] for row in cells)["fail"] == 12


def test_full_context_result_preserves_v8_neutral_and_universal_integrity() -> None:
    ...
    assert all(row["verification"]["localization_fraction"] == 1.0 for row in outcomes)


def test_full_context_result_keeps_repeated_context_failure_visible() -> None:
    outcomes = _load()["outcomes"]
    counts = {
        context: Counter(

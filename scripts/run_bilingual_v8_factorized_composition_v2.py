...
from earshift_bakeoff.bilingual_v8_factorized_composition import (
    FACTORIZED_COMPOSITION_VERSION,
    render_factorized_composition_candidate,
)
from earshift_bakeoff.config import ROOT, stable_json
from earshift_bakeoff.util import atomic_write_json, sha256_file


PROTOCOL_PATH = ROOT / "rules" / "bilingual-v8-factorized-composition-v2.json"
BASELINE_DIR = (
    ROOT
    / "artifacts"
    / "product-matrix"
...
def main() -> int:
    if (RUN_DIR / "results.json").exists():
        raise FileExistsError("factorized composition already has a frozen result")
    ...
    for fixture in protocol["fixtures"]:
        ...
        candidate = render_factorized_composition_candidate(runtime, fixture["text"])
        actual_counts = {
            cell.rule_id: sum(
        ...
        factorized_columns = candidate.render.prosody["rule_decoder_columns"]
        column_sets = [set(values) for values in factorized_columns.values()]
        factorization_contract_pass = bool(
            candidate.render.prosody["version"] == FACTORIZED_COMPOSITION_VERSION
            and set(factorized_columns) == set(actual_counts)
            and all(column_sets)
            and sum(len(values) for values in column_sets)
            == len(set().union(*column_sets))
        )
        contract_pass = bool(
            plan_contract_pass
            and neutral_baseline_bit_exact
            and factorization_contract_pass
        )
        ...
        row = {
            ...
            "neutral_baseline_bit_exact": neutral_baseline_bit_exact,
            "factorization": {
                "version": FACTORIZED_COMPOSITION_VERSION,
                "rule_decoder_columns": factorized_columns,
                "contract_pass": factorization_contract_pass,
            },
        ...
        row["lens_changed_from_v1"] = bool(
            row["audio"]["lens"]["pcm_sha256"]
            != row["baseline_v1"]["lens_pcm_sha256"]
        )
    ...
    result = {
        ...
        "classification": (
            "factorized_composition_pass_eligible_for_disabled_integration_pending_human_qc"
            if all_pass
            else "factorized_composition_incomplete_preserve_exact_failure"
        ),

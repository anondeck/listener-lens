...
PROTOCOL_PATH = ROOT / "rules" / "bilingual-v8-composition-spike-v1.json"
RUN_DIR = (
    ROOT
    / "artifacts"
    / "product-matrix"
...
def main() -> int:
    if (RUN_DIR / "results.json").exists():
        raise FileExistsError("composition spike already has a frozen result")
    ...
    for fixture in protocol["fixtures"]:
        ...
        candidate = runtime.render_v8_composition_candidate(fixture["text"])
        actual_counts = {
            cell.rule_id: sum(
        ...
        expected_counts = fixture["selected_rule_occurrences"]
        contract_pass = bool(
    ...
    result = {
        ...
        "classification": (
            "exploratory_v8_composition_pass_eligible_for_disabled_runtime_integration_pending_human_qc"
            if all_pass
            else "exploratory_v8_composition_incomplete_preserve_exact_failure"
        ),
